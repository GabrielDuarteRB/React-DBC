...
import { ListaDeitada } from "../../components/Lista/Lista"
import { Post, PostLateralSubtopicos, PostLatest } from '../../components/Post/Post'
...
const Ciencia = () =>{
    ...
    const api = async () => {
        try {
            const {data} = await axios.get('https://api.nytimes.com/svc/topstories/v2/science.json?api-key=3PqlwJMMQnNMyrgjuqPAQsdarujrJSkA')
    ...
    return(
        <div className={styles.ciencia}>
            ...
            />
            <div>
                <ListaDeitada itens={categorias}/>
            </div>
            <div className={styles.postCiencia}>
                <div>
                    {
                        informacoes.slice(1, 2).map((item, index) => (
                                <Post
                                    key={index}
                                    titulo={item.title}
                ...
                <div>
                    {
                        informacoes.slice(2, 5).map((item, index) => (
            ...
            <div>
                {
                    informacoes.slice(5, informacoes.length).map((item, index) => (
                            <PostLatest
