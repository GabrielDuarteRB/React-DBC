...
const PostDeitado = ({criador, titulo, conteudo, idElemento}) => {
  ...
  return (
    <div id={idElemento} onClick={() => abrirPost()} className={styles.postDeitado}>
          <span>{criador}</span>
          <h3>{titulo}</h3>
          <small>{conteudo}</small>
    </div>
...
const PostLatest = ({titulo, conteudo, urlImagem, altura, largura, criador, tempo, idElemento}) => {
  ...
  return (
    <div id={idElemento} onClick={() => abrirPost()} className={styles.postLatest}>
      <span>{tempo}</span>
      <div>
        <h1>{titulo}</h1>
        <p>{conteudo}</p>
...
const PostApi = ({api}) => {
  ...
  return (
    <div className={styles.postApi}>
        <div>
            <h3>{api.title}</h3>
            <small>{api.abstract}</small>
            <p>{api.byline}</p>
        </div>
        <img 
          style={{width: '100%', height: '478px'}} 
          src={api.multimedia[1].url}
          alt='Imagem'
        />
    </div>
)
...
export {Post, PostDeitado, PostLateralSubtopicos, PostLatest, PostApi}