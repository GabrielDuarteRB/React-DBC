import { ListaDeitada } from "../Lista/Lista"
import { Post, PostLateralSubtopicos, PostLatest } from "../Post/Post"
...
const SubPosts = ({categorias, pagina}) => {
    ...
    const api = async () => {
        try {
            const {data} = await axios.get(`https://api.nytimes.com/svc/topstories/v2/${pagina}.json?api-key=3PqlwJMMQnNMyrgjuqPAQsdarujrJSkA`)
